...
if ($dataProductos = $producto->readAll()) {
    ...
    foreach ($dataProductos as $rowProductos) {
        ...
        $pdf->cell(0, 10, $pdf->encodeString('Productos: ' . $rowProductos['nombre_producto']), 1, 1, 'C', 1);
        // Se instancia el módelo Producto para procesar los datos.
        $valoracion = new Valoracion;
        // Se establece la marca para obtener sus productos, de lo contrario se imprime un mensaje de error.
        if ($valoracion->setProducto($rowProductos['idcliente'])) {
            // Se verifica si existen registros para mostrar, de lo contrario se imprime un mensaje.
            if ($dataProductos = $producto->ValoracionProductos()) {
                // Se recorren los registros fila por fila.
                ...
                $pdf->cell(0, 10, $pdf->encodeString('No hay valoraciones del producto'), 1, 1);
            }
        } else {
    ...
    $pdf->cell(0, 10, $pdf->encodeString('No hay valoraciones para mostrar'), 1, 1);
}
